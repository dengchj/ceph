...
int RGWRados::transition_obj_to_extra(RGWBucketInfo& bucket_info, rgw_obj& obj, const rgw_placement_rule& placement_rule)
{
  ...
  ldout(cct, 20) << __func__ << " placement name:" << placement_rule.name
  ...
  ldout(cct, 20) << __func__ << " storage_class->endpoint:" << *storage_class->endpoint
                 << " storage_class->dest_bucket:" << *storage_class->dest_bucket
                 << " storage_class->access_key:" << (*storage_class->access_key).id
                 <<dendl;
